<template>
    <footer class="flexbox" :class="{'isFull':footer.status == 2}" v-show="footer.isShow">
        ...
        <a v-show="footer.status == 1 && classType == 2" :href="url" class="cancelOrder applyTrade" @click="applyTrade1">
            <span>去支付</span>
        </a>
         <a v-show="footer.status == 1 && classType == 1" :href="url" class="cancelOrder applyTrade" @click="applyTrade">
            <span>去支付</span>
        </a>
        <a v-show="footer.status == 2" :href="url" class="flex greenBtn isRadius" @click="applyTrade">
            <span v-text="paySuccess"></span>
        </a>
        <a v-show="footer.status == 3" :href="url" class="flex greenBtn isRadius" @click="applyTrade">
            <span>再次购买</span>
        </a>
        <a v-show="footer.status == 4 && classType == 1" :href="url" class="cancelOrder applyTrade" @click="applyTrade">
            <span>去支付</span>
        </a>
        <!--toast提示组件-->
...
export default {
    ...
    methods: {
        ...
        applyTrade () {
            ...
            if (!Common.getQueryString('classType')) {
                ...
                if (parseInt(classType) === 1) {
                    this.url = 'policyDetail2.html?detailId=' + detailId;
                } else if (parseInt(classType) === 2) {
                    this.url = 'policyDetail.html?detailId=' + detailId;
                }
                return false;
            }
            if (Common.isLogin()) {
        ...
        applyTrade1 () {
            if (Common.isLogin()) {
                if (this.footer.status === 1) {
                    this.url = './paymentConfirm.html?tradeId=' + this.tradeId + '&referrerUserId=' + Common.getQueryString('referrerUserId');
                } else if (this.footer.status === 2) {
                    this.url = './index.html';
                } else if (this.footer.status === 3) {
                    this.url = './index.html';
